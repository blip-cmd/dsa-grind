...
if __name__ == "__main__":
    ...
    assert res1 == [[3], [9, 20], [15, 7]], "Failed Test 1"
...
# key: BFS with level size tracking via queue
# notes: Queue length is recorded at the start of each level iteration to process all nodes of that level.
# space: O(N) where N is the maximum width of the tree, which is O(N) in the worst case.
# time: O(N) as each node is enqueued and dequeued exactly once.
